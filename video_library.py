...
def add_comment(video_number, comment):
    if video_number in library:
        if video_number not in comments:
            comments[video_number] = []
        comments[video_number].append(comment)
        return True
    return False

def get_comments(video_number):
    return comments.get(video_number)

def video_exists(video_number):
    return video_number in library
